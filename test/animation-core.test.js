...
function createTestPet(durations) {
...
test("AnimationPlayer gives every frame its own full deadline", () => {
  ...
  player.loadPet(createTestPet([10, 20]));
  player.start("idle");
...
test("AnimationPlayer advances at most one frame per tick", () => {
  ...
  player.loadPet(createTestPet([10, 20, 30]));
  player.start("idle");
...
test("long scheduling gaps preserve the current frame remaining time", () => {
  ...
  player.loadPet(createTestPet([1000, 100]));
  player.start("idle");
